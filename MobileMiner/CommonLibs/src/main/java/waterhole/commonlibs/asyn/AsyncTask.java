 ...
 * <p>
 * There are 2 changes in this copy of AsyncTask:
...
public abstract class AsyncTask<Params, Progress, Result> {
    ...
    /**
     * Executor是线程池接口,真正实现类为ThreadPoolExecutor
     * 参数介绍:
     * int corePoolSize, 核心线程数量,默认核心线程是一直存活的,但是ThreadPoolExecutor如果设置了setCoreThreadTimeout,
     * 那么核心线程在等待keepAliveTime后会终止
     * <p>
     * int maximumPoolSize, 最大线程数
     * <p>
     * long keepAliveTime, 非核心线程等待超时时间,如果设置了setCoreThreadTimeout,则也作用于核心线程
     * <p>
     * TimeUnit unit, 超时单位,为枚举
     * <p>
     * BlockingQueue<Runnable> workQueue, 任务队列
     * <p>
     * ThreadFactory threadFactory, 线程工厂
     * <p>
     * RejectedExecutionHandler handler 超过最大线程数的处理策略
     * <p>
     * 线程池执行流程:
     * 1. 任务来临,如果有空闲核心线程,用一个空闲核心线程去执行
     * 2. 无空闲核心线程,任务队列未满,插入任务队列等待执行
     * 3. 任务队列已满,未达到最大线程数,新开一线程执行任务
     * 4. 达到最大线程数,RejectedExecutionHandler
     */
     ...
     */
    private static final Executor SERIAL_EXECUTOR = APIUtils.hasHoneycomb() ? new SerialExecutor() :
     ...
     */
    @SuppressWarnings({"UnusedDeclaration"})
    private void onProgressUpdate(Progress... values) {
    }
     ...
     * @see #doInBackground
     */
    private final void publishProgress(Progress... values) {
        if (!isCancelled()) {
